...
class GameState(object):
    def __init__(self, Filename, csv_object):
        # 输入参数Filename类型('2018530151256 01-05-03-03-02 推演室9',)有误，是元组
        # csv_object中的Filename是string类型
        ...
        # print("csv_object_red pre", csv_object_red)
        csv_object_red = csv_object_red.sort_values(by="ObjPos")
        # print("csv_object_red next", csv_object_red)
    ...
    def get_score(self, row, SoldierID):
        ...
        if not csv_object_ObjValue.empty:
            ObjValue = csv_object_ObjValue.ObjValue.astype(int)
        else:
            raise Exception
        if SoldierID != 0:
            SoldierValue = (self.csv_object.loc[
                (self.csv_object["Filename"] == row.Filename) & (self.csv_object["ID"] == SoldierID)].ObjValue).astype(int)
        else:
            SoldierValue = 0
