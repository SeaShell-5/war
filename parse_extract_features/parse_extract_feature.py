...
def get_para(csv_roomrecord, csv_object, LOS, map_height, map_urban, map_road, map_normal):
    ...
    para = [(Filename, one_game_group, csv_object, LOS, map_height, map_urban, map_road, map_normal) for
            (Filename), one_game_group in csv_roomrecord_groupby]
...
def parse_one_game(Filename, one_game_group, csv_object, LOS, *map_args):
    ...
    Filename_str = '_'.join(str(item) for item in parts)
    Filename = '_'.join(Filename)
    if one_game_group.iloc[0]["JmResult"] >= 0:
        game_result = 'redwin'
    else:
        game_result = 'bluewin'
    save_file_name = Filename_str + '_' + game_result
...
if __name__ == '__main__':
    ...
    # spatial feature(空间特征):play's colour 1, friendly 6, enemy 6, observe 2, map 4,
    #                           end_pos 6, all is 25*66*51  地图大小3366
    # global feature(全局特征):stage 20, city 4, friendly 23*6, enemy 23*6,  win 1, all is 301 属性特征 301bit
