...
class BatchEnv(object):
    ...
    def __load_replay__(self, path):
        print("load replay path:", path)
        replay_dict = {}
        replay_dict['ptr'] = 0
        replay_dict['done'] = False
        replay_dict['states_S'] = np.asarray(sparse.load_npz(path[0]).todense()).reshape([-1, self.all_channels,                                                                                 self.frame_size[0], self.frame_size[1]])
